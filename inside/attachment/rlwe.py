...
from sigma import CurveHomomorphism, MaurerProof, q, O
...
def rlwe_gen():
    ...
    return (f2l(a), f2l(b)), (f2l(s), f2l(k), f2l(e))
...
class RLWEProof:
    ...
    def encode(self, aux, crs):
        ...
        Ex = sum(sum(2**j*aux[2][i][j] for j in range(2))-1*crs[i] for i in range(n))
        Kx = sum(sum(2**j*aux[1][i][j] for j in range(5))-16*crs[i] for i in range(n))
        ...
        for j in range(5):
            ...
            Gs1.extend(
                [[O]*i*5 + [O]*j + [crs[i]] + [O]*(5-j-1) + [O]*5*(n-i-1) for i in range(n)]
            )
            Y1.extend([aux[1][i][j] for i in range(n)])
            Gs1.extend(
                [[O]*i*5 + [O]*j + [aux[1][i][j]] + [O]*(5-j-1) + [O]*5*(n-i-1) for i in range(n)]
            )
        phi1 = CurveHomomorphism(Gs1)

        Y2, Gs2 = list(), list()
        for j in range(2):
            Y2.extend([aux[2][i][j] for i in range(n)])
            Gs2.extend(
                [[O]*i*2 + [O]*j + [crs[i]] + [O]*(2-j-1) + [O]*2*(n-i-1) for i in range(n)]
            )
            Y2.extend([aux[2][i][j] for i in range(n)])
            Gs2.extend(
                [[O]*i*2 + [O]*j + [aux[2][i][j]] + [O]*(2-j-1) + [O]*2*(n-i-1) for i in range(n)]
            )
