...
每道题直接放在仓库根目录 `<challenge>/`（**不再按 Crypto/Pwn/Misc 分类嵌套**），
遵循 `reference/creating-ctf-docker/SKILL.md`（r3ctf-2025 同款约定）：
...
- `.DS_Store` / `**/._*`（macOS 垃圾）。
- 大文件：
  - `whisper/attachment/whisper-local-stack.7z`（481M）→ 走网盘，
    占位 `whisper/attachment/whisper-local-stack.txt`（**链接待补**）。
  - `Time Capsule/attachment/`（未压缩目录）→ 仓库里只保留压缩好的
    `attachment.7z`。
