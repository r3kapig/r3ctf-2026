...
async def inject_service_assertion(workload_api: client.ApiClient, team_id: int, conn_logger: logging.Logger):

    core_api = client.CoreV1Api(workload_api)
    ns = "platform-operations"
    flag = flag_generate(team_id)
...
async def get_service_account_token(api: client.ApiClient, conn_id: str, team_id: int,  conn_logger: logging.Logger):
    ...
    await inject_service_assertion(api, team_id, conn_logger)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    kubeconfig = await get_service_account_token(api, conn_id, team_id, conn_logger)
                    ...
                    # kubeconfig is a complete apiVersion:v1/kind:Config document
                    # (see kubeconfig.create_kube_token_and_server). The BEGIN/END
                    # markers let the ret.sh bridge pod extract it unambiguously
                    # from the surrounding banner text.
                    writer.write(b"Your kubeconfig is below:\n")
                    writer.write(b"-----BEGIN KUBECONFIG-----\n")
                    writer.write(kubeconfig.encode())
                    writer.write(b"-----END KUBECONFIG-----\n")
