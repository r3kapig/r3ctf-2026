...
from bake_decoys import bake_decoys
...
def check_free_space(image_path, sys_abs_offset, needed_bytes=1024 * 1024):
    ...
    print(f"[bake_image] ext4: block_size={block_size} free_blocks={free_blocks} ({free_mb:.1f} MiB free)")
    if free_bytes < needed_bytes:
        die(f"ext4 has only {free_bytes} free bytes; need at least {needed_bytes}")
    return block_size, free_blocks

def remove_if_present(tmp_ext4, device_path, label):
    ...
    if "File not found" not in combined and "Ext2 directory" not in combined:
        if "Inode:" in combined:
            print(f"[bake_image] Removing existing {device_path} from ext4 ({label}) ...")
            run(["debugfs", "-w", "-R", f"rm {device_path}", tmp_ext4])

def bake_image(image_path, flag_value, whisperd_path=None):

    print(f"[bake_image] Image:    {image_path}")
    print(f"[bake_image] Flag:     [REDACTED ({len(flag_value)} bytes)]")
    if whisperd_path:
        print(f"[bake_image] whisperd: {whisperd_path}")
        wsize = os.path.getsize(whisperd_path)
        print(f"[bake_image] whisperd size: {wsize} bytes ({wsize/1024:.1f} KiB)")
    ...
    print(f"[bake_image] GPT: super at byte offset {super_byte_offset:#x}, size {super_size/1024/1024:.1f} MiB")

    sys_abs_offset, sys_size = find_system_ext4(image_path, super_byte_offset)
    print(f"[bake_image] LP:   system ext4 at byte offset {sys_abs_offset:#x}, size {sys_size/1024/1024:.1f} MiB")

    needed = 1024 * 1024
    if whisperd_path:
        needed = max(needed, os.path.getsize(whisperd_path) * 2)
    check_free_space(image_path, sys_abs_offset, needed)
    ...
    try:
        ...
        print(f"[bake_image] Carving system ext4 to {tmp_ext4} ...")
        run([
            "dd",
            f"if={image_path}",
            f"of={tmp_ext4}",
        ...
        remove_if_present(tmp_ext4, "/flag.txt", "flag")

        cmds_flag = f"""write {tmp_flag} /flag.txt
...
"""
        ...
        print("[bake_image] Injecting /flag.txt via debugfs ...")
        ...
        if actual_out != flag_value.strip():
            die(f"Flag content mismatch.\n  Expected: {flag_value!r}\n  Got: {actual_out!r}")

        print(f"[bake_image] /flag.txt: mode=0600 uid=0 gid=0 content={repr(actual_out[:30])}...")

        if whisperd_path:
            remove_if_present(tmp_ext4, "/system/bin/whisperd", "whisperd")

            cmds_whisperd = f"""write {whisperd_path} /system/bin/whisperd
sif /system/bin/whisperd mode 0100755
sif /system/bin/whisperd uid 0
sif /system/bin/whisperd gid 0
"""
            with open(tmp_cmds, "w") as fh:
                fh.write(cmds_whisperd)

            print("[bake_image] Injecting /system/bin/whisperd via debugfs ...")
            run(["debugfs", "-w", "-f", tmp_cmds, tmp_ext4])

            ws = run(
                ["debugfs", "-R", "stat /system/bin/whisperd", tmp_ext4],
                capture=True,
            )
            ws_out = ws.stderr + ws.stdout
            if "Mode:  0755" not in ws_out or "User:     0" not in ws_out:
                die(f"whisperd stat check failed:\n{ws_out}")

            print("[bake_image] /system/bin/whisperd: mode=0755 uid=0 gid=0. OK.")

        bake_decoys(tmp_ext4)

        print(f"[bake_image] Writing modified ext4 back to image at offset {sys_abs_offset:#x} ...")
        run([
            "dd",
            f"if={tmp_ext4}",
            f"of={image_path}",
        ...
        print("[bake_image] Write-back complete.")

    finally:
        for fname in [tmp_ext4, tmp_flag, tmp_cmds]:
            try:
                os.remove(fname)
            except OSError:
                pass
    ...
    print("[bake_image] Final verification (reading back from image)...")
    ...
    try:
        ...
        print("[bake_image] PROOF:")
        for line in (verify_stat.stderr + verify_stat.stdout).splitlines():
            if any(k in line for k in ("Inode:", "Type:", "Mode:", "User:", "Group:", "Size:")):
                print(f"  flag stat: {line.strip()}")
        print(f"  cat /flag.txt => {final_content!r}")

        if whisperd_path:
            verify_ws = run(
                ["debugfs", "-R", "stat /system/bin/whisperd", verify_tmp],
                capture=True,
            )
            print("[bake_image] whisperd in image:")
            for line in (verify_ws.stderr + verify_ws.stdout).splitlines():
                if any(k in line for k in ("Inode:", "Type:", "Mode:", "User:", "Group:", "Size:")):
                    print(f"  whisperd stat: {line.strip()}")

        print("[bake_image] SUCCESS: /flag.txt and /system/bin/whisperd baked.")
    finally:
        try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Offline artifact injection into Android AVD system.img"
    )
    parser.add_argument("--image", required=True, help="Path to a WRITABLE COPY of system.img")
    parser.add_argument("--flag", required=True, help="Flag value to bake in (e.g. CTF{...})")
    parser.add_argument(
        "--whisperd",
        default=None,
        help="Path to Android x86_64 whisperd binary to bake into /system/bin/whisperd",
    )
    ...
    if args.whisperd and not os.path.isfile(args.whisperd):
        die(f"whisperd binary not found: {args.whisperd}")
    ...
    bake_image(args.image, args.flag, args.whisperd)
