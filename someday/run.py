...
def build_provision_script(admin_password_ps: str, hacker_password_ps: str) -> str:
    return (
        ...
        "  # Securely remove the bundled vhdmp.exe: overwrite its clusters in place\n"
        "  # (NTFS reuses the same clusters for an in-place overwrite), then delete it,\n"
        "  # so it cannot be recovered by reading the raw volume after escalation.\n"
        "  $vh = 'C:\\Users\\Public\\tmp\\vhdmp.exe'\n"
        "  if (Test-Path -LiteralPath $vh) {\n"
        "    try {\n"
        "      $vhLen = (Get-Item -LiteralPath $vh).Length\n"
        "      if ($vhLen -gt 0) {\n"
        "        $vhFs = [System.IO.File]::Open($vh, [System.IO.FileMode]::Open, [System.IO.FileAccess]::ReadWrite)\n"
        "        $vhBuf = New-Object byte[] $vhLen\n"
        "        $vhFs.Write($vhBuf, 0, $vhLen)\n"
        "        $vhFs.Flush($true)\n"
        "        $vhFs.Close()\n"
        "      }\n"
        "      Remove-Item -LiteralPath $vh -Force\n"
        "    } catch {}\n"
        "  }\n"
