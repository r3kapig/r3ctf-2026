...
# Per-team dynamic flag parameters. The flag is derived from the ret.sh team id
# so every team gets a different flag (anti-sharing). For the platform to accept
# the submitted flag, FLAG_SALT and FLAG_CHAL_ID must match the checker.rx
# ENCRYPT_KEY / HASH_KEY this challenge is configured with on ret.sh — override
# them via env when the real salt / challenge-id differ from the defaults below.
...
def flag_generate(team_id: int) -> str:
    """Per-team flag via UUIDStego (key=FLAG_SALT, template=FLAG_CHAL_ID).

    Embeds the ret.sh team id into a UUID and prefixes it with `sk-`.
    Deterministic: same (team_id, salt, chal_id) -> same flag. The platform
    decodes the UUID (after stripping `sk-`) back to team_id for anti-sharing;
    a different team_id yields a different flag.
    """
