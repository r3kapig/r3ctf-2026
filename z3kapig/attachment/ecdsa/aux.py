...
def _find_small_prime_factor(n: int) -> int | None:
...
class Aux:
    ...
    def __init__(self, party_id: int, ec: ECOperations, ssid: int):
        ...
        self.lamd = gmpy2.mpz(random_positive_int(self.paillier_priv_i.phi_n))
        r = get_random_positive_relatively_prime_int(self.paillier_pub_i.n)
        self.rhoi = gmpy2.mpz(random_positive_int(self.paillier_priv_i.n))
    ...
    def round3(
        ...
        modulus_bits = msg_j_r2.n.bit_length()
        if msg_j_r2.n <= 0 or modulus_bits != PAILLIER_MODULUS_BITS:
            raise VerificationError(
                f"Other party's Paillier N must be {PAILLIER_MODULUS_BITS} bits, "
                f"got {modulus_bits} bits"
            )

        small_factor = _find_small_prime_factor(msg_j_r2.n)
        if small_factor is not None:
            raise VerificationError(
                "Other party's Paillier N is divisible by small prime "
                f"{small_factor}"
            )
