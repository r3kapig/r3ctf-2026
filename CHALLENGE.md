...
- **Mafuyuuuuu-rev**：Mafuyuuuuu 的 **revenge 版**（独立题目，见 `Mafuyuuuuu-rev/`），结构同 Mafuyuuuuu（三容器单 pod），区别在 backend `PaperTrailDesk.dll` 加了 signal 限流（429）+ frontend `main.js`/`styles.css` 加了 2s 发送冷却。镜像 `mafuyuuuuu-rev-{backend,frontend,nginx}:latest`，`k8s.yaml` Service `mafuyuuuuu-rev`、NodePort **30090**。动态 flag 机制同 Mafuyuuuuu（未动）。
