...
| speedrun | `registry.ctf2026.r3kapig.com/r3ctf_2026_6a511700/speedrun-{minecraft,checker}:latest`（2 镜像，单 pod 共享） | 2→6 | 4g→8g | pushed ✓ |
| blinky | `registry.ctf2026.r3kapig.com/r3ctf_2026_6a511700/blinky:latest` | 2.0 | 1g | pushed ✓ |
| minemaze | `registry.ctf2026.r3kapig.com/r3ctf_2026_6a511700/minemaze:latest` | 2.0 | 4g | pushed ✓ |
...
- **minemaze**：Minecraft（Folia 26.1.2 / Java 25）misc，单容器（`eclipse-temurin:25-jre`），监听 TCP **25565**，motd `MineMaze`、`online-mode=false`（vanilla 1.21.x 客户端直连即可，无需账号）、`max-players=2`。`RekaMaze` 插件生成 200×200 的「迷雾 + 锁与钥匙」迷宫（8 道门 / 8 把钥匙，只渲染玩家周围半径 8 格的 bubble），选手 `/startgame` 后探索、找钥匙、开门、到终点拿 flag；命令 `/startgame`、`/leavegame`，admin `/rekamaze <reload|stop <player>|info>`。同服还装了 `GrimAC` 反作弊。**动态 flag**：平台注入 `$FLAG`，`RekaMaze` 插件的 `FlagService` 直接读环境变量并在通关时颁发（未配置时回退占位 `r3ctf{FLAG_NOT_CONFIGURED}`，所以**必须**传 `FLAG`）。JVM `-Xms1G -Xmx3G`，容器给 ~4 GB 内存、~2 CPU。镜像是作者提供的 `rekamaze-folia:latest`（预构建 docker-save tar，server jar / 插件 / world datapack / 配置全 bake 在内），retag 为 `minemaze:latest` 推送。本地 `infra.sh` 映 `25565:25565`。**选手附件**（`attachment/`）：`gmtoolbox-1.0.0.jar`（Fabric mod，客户端需装 Fabric Loader + Fabric API + 此 mod，见 `player_rules.pdf`）+ `player_rules.pdf`（规则/客户端要求）。**源码**：`minemaze/source/` 是作者的 `rekamaze-folia` 构建上下文（Dockerfile + server 配置 + `RekaMaze` 插件 jar + maze datapack），仅供参考（镜像已构建推送），其中 folia jar / libraries / cache / versions / GrimAC 等第三方大文件未入库（见 `source/README.md` + `.gitignore`）。
