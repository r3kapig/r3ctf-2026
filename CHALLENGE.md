...
- **Mafuyuuuuu**：.NET web（Project SEKAI 主题），**三容器单 pod** 部署（见 `Mafuyuuuuu/deploy/k8s.yaml`）：`backend`（ASP.NET Core 8，`PaperTrailDesk.dll`，8080）/ `frontend`（Vite，4173）/ `nginx`（8089，对外）。三个容器共享 pod 网络命名空间，nginx 反代到 `127.0.0.1:4173`（frontend）和 `127.0.0.1:8080`（backend，配置在 `nginx/nginx.pod.conf`，通过 ConfigMap 挂载）。push 的是 3 个镜像 `mafuyuuuuu-{backend,frontend,nginx}:latest`。`k8s.yaml` 里 Service 用 NodePort **30089**（可按平台改）；flag 通过 ConfigMap `mafuyuuuuu-flag` 挂载到 backend 的 `/flag`（`/readflag` setuid 读取），平台可换成动态 flag。本地 dev 仍可用 `infra.sh`（docker compose，用 `nginx/nginx.conf` 的 service-name 反代）。
