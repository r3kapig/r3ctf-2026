...
def _persist_identity(handle, phone):
    _write_file(HANDLE_FILE, handle)
    _write_file(PHONE_FILE, phone)
    log.info("Persisted identity: handle=%s, phone=%s", handle, phone)
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _do_reset(self, body):
        ...
        if new_handle:
            _persist_identity(new_handle, new_phone)
            log.info("Reset: new lease identity handle=%s, phone=%s", new_handle, new_phone)
        else:
            log.info("Reset: self-heal path -- using current handle=%s", _current_handle())
